...
$draft.To = "jason.clement@example.com"
...
<p>Here is what I want to do about it. I am launching a nonprofit called Sidewalk Stories LA. The idea is to turn those broken sidewalk panels into small public mosaics, designed and built by neighborhood kids working alongside a professional teaching artist and a licensed concrete crew. Residents snap a photo of a damaged section through an app, the community votes on which one matters most, and each month the top panel gets repaired and turned into a mosaic. Part civic fix, part public art, part youth program.</p>
...
<p>1. <strong>One Page Funder Prospectus</strong> — the pitch we are using with grant officers and sponsors<br>
2. <strong>Financial Model</strong> — the Pilot Scenario shows a 10 panel Year 1 plan that is actually fundable<br>
3. <strong>Partnership Outline</strong> — three ways we could structure your role, from board member to advisor</p>
...
# Attach files
$attachments = @(
    "C:\Users\tdamy\OneDrive - CLS CRE\CLS CRE\Brokerage\AI - LLMs\Claude Code\Projects Marketplace\Sidewalk Art\One-Page-Funder-Prospectus.md",
    "C:\Users\tdamy\OneDrive - CLS CRE\CLS CRE\Brokerage\AI - LLMs\Claude Code\Projects Marketplace\Sidewalk Art\Sidewalk_Mosaic_Nonprofit_Financial_Model.xlsx",
    "C:\Users\tdamy\OneDrive - CLS CRE\CLS CRE\Brokerage\AI - LLMs\Claude Code\Projects Marketplace\Sidewalk Art\Partnership-Outline-for-Jason.md"
)

foreach ($file in $attachments) {
    if (Test-Path $file) {
        $draft.Attachments.Add($file) | Out-Null
    }
}

$draft.Save()
Write-Host "Draft saved to Outlook Drafts folder with HTML formatting (11pt) and 3 attachments"
